...
#!/sbin/sh
/etc/fs/ufs/mount -o remount,rw /dev/dsk/${DRIVE1} /
EOF
for xopt in $*
do
...
echo "y" | env NOINUSE_CHECK=1 /usr/sbin/newfs /dev/rdsk/$DRIVE1
/usr/sbin/mount /dev/dsk/$DRIVE1 ${ALTROOT}

echo "Copying main filesystems"
cd ${DESTDIR}
tar cf - . | ( cd $ALTROOT ; tar xf -)
cd $ALTROOT
touch etc/mnttab
mkdir -p dev/fd devices/pseudo opt var var/run mnt
${DESTDIR}/usr/sbin/devfsadm -r ${ALTROOT}
rm -f ${ALTROOT}/dev/dsk/* ${ALTROOT}/dev/rdsk/* ${ALTROOT}/dev/usb/h*
rm -f ${ALTROOT}/dev/removable-media/dsk/* ${ALTROOT}/dev/removable-media/rdsk/*
rm -fr ${ALTROOT}/dev/zcons/*
...
touch ${ALTROOT}/boot/grub/bootsign/tribblix_18
echo "tribblix_18" > ${ALTROOT}/etc/bootsign
